...
// ✅ FIX: Use service role key so RLS doesn't block inserts
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 2. ✅ FIX: Generate reference here and include it in the insert
    ...
    const { data: booking, error: bookingError } = await supabase
      ...
      .insert({
        ...
        status: 'pending',
        reference,  // ✅ insert the reference we generated
      })
      .select('reference')
      .single()
          ...
          html: `
            <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px; background: #100F0D; color: #F5EFE3;">
              ...
              <div style="background: rgba(255,255,255,.05); border: 1px solid rgba(255,255,255,.07); border-radius: 12px; padding: 20px; margin-bottom: 24px;">
                ...
                <div style="font-size: 15px; margin-bottom: 16px;">${clientEmail}</div>
